...
def define_triplets(images, labels, nbof_triplet=10000 * 3, datagen=datagen):
    ...
    for i in tqdm_notebook(range(0, nbof_triplet, 3)):
        ...
        triplet_train[i] = single_apply_transform(
            keep_classAP[idx_image1], datagen)
        triplet_train[i +
                      1] = single_apply_transform(keep_classAP[idx_image2], datagen)
        ...
        triplet_train[i +
                      2] = single_apply_transform(keep_classN[idx_image3], datagen)
...
def global_define_hard_triplets(images, labels, predict, datagen=datagen):
    ...
    for i in range(nbof_classes):
        ...
        for j in range(len(keep_predict_class)):
            ...
            dist_class = np.sum(
                np.square(keep_predict_class-keep_predict_class[j]), axis=-1)

            # Add the anchor
            triplets[idx_triplets] = single_apply_transform(
                images[idx_images+j], datagen)
            y_triplets[idx_triplets] = labels[idx_images+j]

            # Add the hard positive
            triplets[idx_triplets+1] = single_apply_transform(
                images[idx_images+np.argmax(dist_class)], datagen)
            y_triplets[idx_triplets +
                       1] = labels[idx_images+np.argmax(dist_class)]

            # Computes the distance between the current vector and the vectors of the others classes
            dist_other = np.sum(
                np.square(predict_other-keep_predict_class[j]), axis=-1)

            # Add the hard negative
            triplets[idx_triplets +
                     2] = single_apply_transform(images[np.argmin(dist_other)], datagen)
...
def define_hard_triplets(images, labels, predict, class_subset_size=10, add=100*3):
    ...
    _, h, w, c = images.shape
    triplets = np.empty(
        (3*len(predict)+add*(nbof_classes//class_subset_size + 1), h, w, c))
    y_triplets = np.empty(3*len(predict)+add *
                          (nbof_classes//class_subset_size + 1))
    idx = 0
    for i in tqdm_notebook(range(0, len(classes), class_subset_size)):
        ...
        keep_classes = np.array(
            [labels[j] in selected_classes for j in range(len(labels))])
        ...
        triplets[idx+length:idx+length+add], y_triplets[idx +
                                                        length:idx+length+add] = define_triplets(images, labels, add)
